...
#Reading in the outputed results from the pipeline pertaining to the validation data
data = pd.read_csv("..\output\mean_max\LIA_max.csv")
...
def cross_validation_ROC(data: pd.DataFrame, metric: str):
    ...
    for training_index, test_index in kf.split(data):
        ...
        yj_optimal_index = np.argmax(tpr - fpr)
        yj_optimal_threshold = thresholds[yj_optimal_index]
        optimal_thresholds.append(yj_optimal_threshold)
        ...
        yj_tpr = tpr[yj_optimal_index]
        yj_tpr_list.append(yj_tpr)
        specificity = 1 - fpr[yj_optimal_index]
        specificities.append(specificity)
        yj_fpr = fpr[yj_optimal_index]
        ...
        for scoring_value in x_test_scoring_metric:
            if scoring_value >= yj_optimal_threshold:
                y_pred.append(1)
            else:
                y_pred.append(0)
        ...
        test_confusion_matrix = metrics.confusion_matrix(y_test, y_pred,labels=[0,1])
    ...
    #plt.title(f'ROC Curves For Each Iteration Of CV For Mean {metric}')
   ...
   # print(f"FPR assocaited with Youden's J statistic:{yj_fpr_list}")
    ...
    #Calculating the average TPR at the optimal threhsold based on Youden's J statistic across the 5 interations of CV
    print(f"Average TPR at the optimal threshold based on Youden's J statistic:{round(np.average(yj_tpr_list),3)} +/- {round(np.std(yj_tpr_list),3)}")
    ...
    print(f"Average test accuracies:{round(average_test_accuracy,3)} +/- {round(sd_test_accuracy,3)}")
